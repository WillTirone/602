...
&p(\psi) = \frac{e^\psi}{1+e^\psi} \frac{1}{(1+e^\psi)^2} = \frac{e^\psi}{(1+e^\psi)^3}\\
...
```{r}
psis = seq(0,5,by=.001)
data = (exp(1)**psis) / (1+exp(1)**psis)**3
...
&p(\psi) = e^{2\psi}e^{-e^\psi}; \psi>0
\end{aligned}
$$

```{r}
data = exp(1)**(2*psis) * exp(1)**(-exp(1)**psis)
plot(psis,data,type='l')
```
